...
defineExpose({
  ...
  getAllFeatures: () => {
    ...
    return Object.fromEntries(
      (Object.entries(tools) as [ToolKey, IDrawTool][]).map(([key, tool]) => [key, tool.getFeatures()]),
    );
  },
  /** 动态修改某工具的样式 */
  updateToolConfig: (key: ToolKey, config: Parameters<IDrawTool['updateConfig']>[0]) => {
    tools?.[key]?.updateConfig(config);
  },
  featureCount,
});
</script>
