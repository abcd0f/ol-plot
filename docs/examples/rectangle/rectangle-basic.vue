<template>
  <div class="map-container">
    ...
    <div v-if="isDrawing" class="status-tip">单击确定起点，再次单击确定对角点</div>
    ...
    <div class="info-panel">
      ...
      <div class="info-row">
        <span class="info-label">宽 / 高</span>
...
import { RectangleTool, DrawEvent } from '../../../packages';
...
function formatArea(geom: OlPolygon): string {
  const area = getArea(geom);
  if (area > 1_000_000) return `${(area / 1_000_000).toFixed(2)} km²`;
  return `${area.toFixed(0)} m²`;
}

function updateSize(geom: OlPolygon) {
  ...
  const fmt = (v: number) => v > 1000 ? `${(v / 1000).toFixed(2)}km` : `${v.toFixed(0)}m`;
  sizeText.value = `${fmt(w)} / ${fmt(h)}`;
}
...
onMounted(() => {
  ...
  tool
    .on(DrawEvent.DRAW_START, () => {
      addLog({ type: 'start', label: '开始', msg: '开始绘制矩形' });
    })
    .on(DrawEvent.DRAW_END, ({ feature }) => {
      ...
      updateSize(geom);
      addLog({ type: 'end', label: '完成', msg: `面积 ${formatArea(geom)}` });
    })
    .on(DrawEvent.SELECT, ({ feature }) => {
      const geom = feature.getGeometry() as OlPolygon;
      updateSize(geom);
      addLog({ type: 'select', label: '选中', msg: `面积 ${formatArea(geom)}` });
    })
    ...
    .on(DrawEvent.MODIFY_END, ({ features }) => {
      const geom = features[0].getGeometry() as OlPolygon;
      updateSize(geom);
      addLog({ type: 'modify', label: '编辑', msg: `面积 ${formatArea(geom)}` });
    });
});
...
.toolbar-btn:hover  { color: #fa8c16; border-color: #fa8c16; background: #fff7e6; }
.toolbar-btn.active { color: #fff; background: #fa8c16; border-color: #fa8c16; }
...
.log-tag--start  { background: #fa8c16; }
.log-tag--end    { background: #d46b08; }
.log-tag--select { background: #faad14; }
.log-tag--modify { background: #722ed1; }
